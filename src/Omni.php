...
class Omni
{
    ...
    public function scaffold()
    {
        ...
        if ($input['omni_response_json'] ?? 0) {
            $routeCalls['middleware'] = [
                'admin.auth:sanctum'
            ];
        } else {
            $routeCalls['middleware'] = [
        ...
        foreach ($input['fields'] as $field) {
            ...
            if (!$row) {
                ...
                $row->rules = '';
                $row->dict = json_encode($dict, JSON_UNESCAPED_UNICODE);
        ...
        if ($input['omni_route_uri']) {
            ...
            $route->calls = json_encode($routeCalls, JSON_UNESCAPED_UNICODE);
            $route->save();
        }
    }
